...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals) + AVG(opponent_goals), 16) AS combined_goals FROM games")"
...
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN (SELECT winner_id AS team_id FROM games WHERE year=2014 AND round='Eighth-Final' UNION SELECT opponent_id FROM games WHERE year=2014 AND round='Eighth-Final') AS t2 ON teams.team_id=t2.team_id ORDER BY name ASC")"
